...
if(isset($_REQUEST['action'])){ // eval to get the state of the form
...
switch ($myAction) {//check 'action' for type of process
	case "confirmed":  // display final confirmed order
		showOrder();
		echo '<br />	<a href="' . THIS_PAGE . '">COMPLETE ORDER</a>'; //Pretend send order on
		break;

	case "display":  // process order
		$myValue = 'submitted'; //submit button title
		showName();
		echo showForm($myState = 'submitted', $mySubscriber, $myValue);

		echo '<br />	<a href="' . THIS_PAGE . '">EXIT ORDER</a>';
		 break;

	default: // start deets
		$myValue = 'display'; //submit button title
		echo showForm($myState = 'display', $mySubscriber, $myValue); //submit button title
		myDump($_POST);
}
		...
		<form
			...
			<select name="Publisher">
				...
				$Publisher = $_POST["Publisher"];
				 ...
				 if (isset($Publisher) && $Publisher == "") { $myForm .= "selected"; }
				//Will want to go overkill later with all publishers....
				//use dataScrape to get all pubs publishing that week...
				$myForm .= '>Select Publisher</option>
				<option value="Marvel"';

				if (isset($Publisher) && $Publisher == "Marvel"){
					$myForm .= "selected"; }
				$myForm .= '> Marvel</option>

				<option value="DC"';
				if (isset($Publisher) && $Publisher == "DC"){
					$myForm .= "selected"; }
				$myForm .= '<option value="Image"';

				if (isset($Publisher) && $Publisher == "Image"){
					$myForm .= "selected"; }
				$myForm .= '>Image</option>
...
function showOrder() {#form submits here we show entered name
